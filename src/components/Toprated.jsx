...
const Toprated = () => {
  ...
  const fetchMovies = useCallback(async () => {
    try {
      ...
      // Only append new movies when loading more pages
  ...
  const shareMovie = useCallback((movie) => {
    const shareData = {
      title: movie.title,
      text: `Check out ${movie.title} (${movie.vote_average}/10), a top rated movie from ${movie.release_date?.substring(0, 4) || 'unknown year'}!`,
  ...
  // Generate year options for filter (last 30 years)
  ...
  // Memoized movie list to display
...
// Extracted Movie Card Component for better reusability
...
// Extracted Trailer Modal Component
